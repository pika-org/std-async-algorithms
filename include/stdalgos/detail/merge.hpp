...
namespace stdalgos {
    namespace merge_detail {
        ...
        struct merge_t
        {
            ...
            template <typename InputIt1, typename InputIt2, typename OutputIt,
                typename... Properties>
            OutputIt operator()(execution_properties<Properties...> const& exec_properties,
                InputIt1 b1, InputIt1 e1, InputIt2 b2, InputIt2 e2, OutputIt d) const
            {
                // Fall back to synchronizing the asynchronous overload if no synchronous
                // customization is available.
                ...
                return std::get<0>(stdexec::this_thread::sync_wait(
                    merge_t{}(stdexec::just(b1, e1, b2, e2, d), exec_properties))
                                       .value());
            }
      ...
      (!stdexec::tag_invocable<merge_t, Scheduler, InputIt1, InputIt1, InputIt2, InputIt2, OutputIt>)
                    ...
                    OutputIt d) const
            {
                ...
                return std::get<0>(stdexec::this_thread::sync_wait(
                    merge_t{}(
                        stdexec::transfer_just(std::forward<Scheduler>(sched), b1, e1, b2, e2, d)))
                                       .value());
            }
      ...
      (!stdexec::tag_invocable<merge_t, Sender, execution_properties<Properties...> const&>)
                ...
                operator()(Sender&& sender,
                    execution_properties<Properties...> const& exec_properties) const
            {
                ...
                if constexpr (stdexec::__has_completion_scheduler<Sender, stdexec::set_value_t>)
                {
                    ...
                    return stdexec::let_value(std::forward<Sender>(sender),
                        [sched = std::move(sched), exec_properties](
                            ...
                            return stdexec::schedule(std::move(sched)) |
                                stdexec::then(
                                    [=]() mutable { return merge_impl(b1, e1, b2, e2, d); });
                        });
                }
                else
                {
