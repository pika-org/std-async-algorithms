...
namespace stdalgos {
    namespace for_each_detail {
        struct for_each_t
        {
            ...
            void operator()(
                execution_properties<Properties...> const& exec_properties, It b, It e, F&& f) const
            {
                // Fall back to synchronizing the asynchronous overload if no synchronous
                // customization is available.
            ...
            {
                return for_each_t{}(
                    std::forward<Sender>(sender), execution_properties<>{}, std::forward<F>(f));
            }
            ...
            requires
                ...
                operator()(Sender&& sender,
                    execution_properties<Properties...> const& exec_properties, F&& f) const
            {
            ...
            {
                return stdexec::tag_invoke(
                    std::forward<Sender>(sender), exec_properties, std::forward<F>(f));
            }
                    ...
                    execution_properties<Properties...> const& exec_properties, F&& f) const
            {
                ...
                if constexpr (stdexec::__has_completion_scheduler<Sender, stdexec::set_value_t>)
                {
                    ...
                    return stdexec::let_value(std::forward<Sender>(sender),
                        [f = std::forward<F>(f), sched = std::move(sched), exec_properties](
                            auto& b, auto& e) {
